...
- Copy 'history.md' to 'docs/external'.
...
( async function ()
{
	...
	log_blank_line();
	log_heading( 'Preflight: Copy history.md to docs/external' );
	path = LIB_PATH.join( process.cwd(), 'docs', 'external', 'history.md' );
	LIB_FS.copyFileSync( LIB_PATH.join( process.cwd(), 'history.md' ), path );
	...
	// 	await execute_command( `aws s3 sync ${DOCS_FOLDER} s3://${AWS_BUCKET}`, { AWS_PROFILE: 'admin' } );
	// }
